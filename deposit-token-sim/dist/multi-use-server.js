...
app.post('/bank/authorize-multiuse', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const { sender, recipient, amount, totalLimit, maxUses = 5, expiration } = req.body;
        // Validate inputs
        ...
        const calculatedTotalLimit = totalLimit
            ? ethers_1.ethers.utils.parseEther(totalLimit)
        ...
        const encodedAuth = ethers_1.ethers.utils.defaultAbiCoder.encode(['address', 'uint256', 'uint256', 'uint256', 'uint256', 'uint256'], [
        ...
        issuedAuthorizations[authHash] = {
            ...
            issuedAt: Date.now(),
            maxUses,
            totalLimit: ethers_1.ethers.utils.formatEther(calculatedTotalLimit)
        };
        const response = {
            ...
            usageInfo: {
                maxUses,
                totalLimit: ethers_1.ethers.utils.formatEther(calculatedTotalLimit),
                perTransactionLimit: ethers_1.ethers.utils.formatEther(amountWei)
            }
        };
        console.log(`Multi-use authorization created for ${sender}`);
        console.log(`Max uses: ${maxUses}, Total limit: ${ethers_1.ethers.utils.formatEther(calculatedTotalLimit)} ETH`);
...
app.get('/bank/authorization/:authHash', (req, res) => {
    ...
    res.json({
        ...
        spendingLimit: ethers_1.ethers.utils.formatEther(authorization.spendingLimit),
        totalLimit,
        maxUses,
...
app.post('/bank/revoke/:authHash', (req, res) => {
    const { authHash } = req.params;
    const authInfo = issuedAuthorizations[authHash];
    if (!authInfo) {
        return res.status(404).json({ error: 'Authorization not found' });
...
app.post('/bank/authorize', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        return app._router.handle(Object.assign(Object.assign({}, req), { url: '/bank/authorize-multiuse', path: '/bank/authorize-multiuse' }), res, () => { });
    }
    catch (error) {
